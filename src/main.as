...
void Main() {
    ...
    while(true) {
        if(Permissions::ViewRecords() && Enabled && ExecuteRefreshNow) {
...
void Update(float dt) {
	...
	if (app.CurrentPlayground !is null && app.RootMap !is null) {
        CurrentlyInMap = true;
	} else {
    ...
    AbsoluteButtonPosition = ButtonPosition * vec2(Draw::GetWidth(), Draw::GetHeight());
